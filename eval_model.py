...
def init_model(args):
    ...
    if args.load == 0:
        ...
        ckp = f'./{args.out_dir}/{modes[args.model_mode]}_{args.dim}{moe_path}_wiki6.pth'        #模型名字
        print(ckp)
        model = miniLLMChineseLM(LMConfig(
            dim=args.dim,
        ...
        transformers_model_path = './mini_LLM_Chinese2'
        tokenizer = AutoTokenizer.from_pretrained(transformers_model_path)
        model = AutoModelForCausalLM.from_pretrained(transformers_model_path, trust_remote_code=True)
    print(f'mini-LLM-Chinese模型参数量: {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.2f}M(illion)')
...
def main():
    parser = argparse.ArgumentParser(description="Chat with mini-LLM-Chinese")
    parser.add_argument('--lora_name', default='lora_medical',help="lora_medical | lora_identity" ,type=str)
