...
def init_model(lm_config):
    ...
    moe_path = '_moe' if lm_config.use_moe else ''
    ckp = f'./out/full_sft_{lm_config.dim}{moe_path}_wiki6.pth'
    state_dict = torch.load(ckp, map_location=args.device)
    model.load_state_dict(state_dict, strict=False)
    # 初始化参考模型
    ref_model = miniLLMChineseLM(lm_config)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="mini-LLM-Chinese RLHF")
    ...
    parser.add_argument("--wandb_project", type=str, default="mini-LLM-Chinese-RLHF-SFT")
    ...
    args.wandb_run_name = f"mini-LLM-Chinese-Full-DPO-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LearningRate-{args.learning_rate}"
